 ...
 * and the zoom slider, and the band under it holds one thin row per task at its
 * full height. Both charts read the same zoom window. The gutter cell
  ...
  const bandOption = useMemo(() => {
    ...
    return {
      ...
      tooltip: {
        ...
        // A box taller than the modal is cut off inside its scroll container,
        // so the body carries it. Its position stays in canvas coordinates, and
        // the clamp holds it in the window through where the canvas sits.
        appendToBody: true,
        position: (point: number[], _params: unknown, dom: HTMLElement, _rect: unknown, size: { viewSize: number[] }) => {
          const canvasTop = bandRef.current?.getBoundingClientRect().top ?? 0
